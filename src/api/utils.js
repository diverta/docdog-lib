...
export function get(uri, params = {}, headers = {}) {
  if (window.KUROCO_API_HOST) {
    return axios.get(window.KUROCO_API_HOST + uri, {
      headers,
      params,
    });
  } else {
    console.error('[Docdog] KUROCO_API_HOST is undefined. Please check your Google Tag Manager settings');
  }
}

export function post(uri, post_data = {}, headers = {}) {
  if (window.KUROCO_API_HOST) {
    return axios.post(window.KUROCO_API_HOST + uri, JSON.stringify(post_data), {
      headers,
    });
  } else {
