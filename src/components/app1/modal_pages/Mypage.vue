<template>
  ...
  <div class="kuroco-container--middle">
    ...
    <section class="kuroco-modal__body__section kuroco-container--white">
      <h2 class="kuroco-modal__body__heading">お知らせ</h2>
      <ul class="kuroco-list">
        <li class="kuroco-list__item" v-for="news in listNews">
          <button type="button" class="kuroco-list__item__link" @click.prevent="redirect({ target: 'NewsDetails', params: { data: news} })">
            <time datetime="2022-03-01">{{ news.ymd }}</time>
            <span class="kuroco-badge">{{ getCategoryName(news.contents_type) }}</span>
            <span class="kuroco-list__item__title">{{ news.subject }} </span>
          </button>
        </li>
      </ul>
    </section>
    <section class="kuroco-modal__body__section kuroco-container--white">
      <h2 class="kuroco-modal__body__heading">あなたにおすすめのコンテンツ</h2>
      <ul class="kuroco-list">
        <li class="kuroco-list__item kuroco-u-py-lg">
          <h3 class="kuroco-modal__body__sub-heading">動画</h3>
          <ul class="kuroco-card__list" v-if="videos.length > 0">
        ...
        <li class="kuroco-list__item kuroco-u-py-lg">
          <h3 class="kuroco-modal__body__sub-heading">記事</h3>
          <ul class="kuroco-card__list" v-if="topics.length > 0">
        ...
        <li class="kuroco-list__item kuroco-u-py-lg">
          <h3 class="kuroco-modal__body__sub-heading">資料</h3>
          <ul class="kuroco-card__list" v-if="docs.length > 0">
    ...
    <section class="kuroco-modal__body__section kuroco-container--white" v-if="isLogin">
      <h2 class="kuroco-modal__body__heading">アカウント情報</h2>
